...
const TestPage = () => {
  ...
  const requestMediaAccess = useCallback(async () => {
    ...
    try {
      ...
      setMediaError("Camera and microphone access is required to start this proctored test.");
      throw err;
    } finally {
  ...
  // Release the camera/mic once the test page is done with them
  useEffect(() => {
          ...
          <button
            onClick={async () => {
              try {
                ...
                return; // stay on this screen — mediaError is now set, shown below
              }
              await requestFullscreen();
              setTestStarted(true);
            }}
            disabled={requestingMedia}
            className="txp-btn-fill w-full flex items-center justify-center gap-2 text-white text-sm font-semibold py-3 rounded-lg disabled:opacity-60"
          >
            <FaExpand size={13} />
            {requestingMedia ? "Requesting camera access..." : "Allow camera & start test"}
          </button>

          {mediaError && (
            <p className="text-[#A83A34] text-xs text-center mt-3">{mediaError}</p>
          )}
        </div>
      </div>
    );
