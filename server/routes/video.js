...
//         const newPlaylist = await Playlist.create({
//             name: title.trim(),
//             videos: [],          // no video yet — added later from the dashboard
//             userid: userId,
//         });

//         res.status(200).send({ playlist: newPlaylist });
//     } catch (error) {
//         console.error(error);
//         res.status(500).send("Server Error");
//     }
...
// Deletes an old Cloudinary asset when it's being replaced. Swallows errors so a
// stale/missing asset (e.g. already deleted, or an old record with no public_id)
// never blocks the DB update itself.
const destroyOldAsset = async (publicId, resourceType) => {
    if (!publicId) return;
    try {
        await cloudinary.uploader.destroy(publicId, { resource_type: resourceType });
    } catch (error) {
        console.error(`Failed to delete old ${resourceType} (${publicId}):`, error.message);
    }
};
...
router.post("/uploadtoplaylist/:id", fetchuser, async (req, res) => {
    try {
        const playlistId = req.params.id;
        const userId = req.user.id;
        const { title, videoURL, thumbnailURL, videoPublicId, thumbnailPublicId } = req.body;
...
// Edit an existing video: change title, and/or replace thumbnail, and/or replace
// the video file itself. The client uploads the new file(s) directly to Cloudinary
// first (same /upload-signature flow used for creation), then calls this route with
// whichever of videoURL/thumbnailURL actually changed. Old assets are deleted here.
router.patch("/editvideo/:id", fetchuser, async (req, res) => {
    try {
        const videoId = req.params.id;
        const userId = req.user.id;
        const { title, videoURL, videoPublicId, thumbnailURL, thumbnailPublicId } = req.body;
        ...
        // 1. Title Update (Optional/Independent)
        if (title && title.trim()) {
            video.title = title.trim();
        }

        // 2. Video File Update (Optional/Independent)
        // Only run if videoURL is provided. We also verify videoPublicId exists before clearing the old asset.
        if (videoURL) {
            if (video.videoPublicId) {
                await destroyOldAsset(video.videoPublicId, "video");
            }
            video.url = videoURL;
            video.videoPublicId = videoPublicId || null;
        }

        // 3. Thumbnail Image Update (Optional/Independent)
        // Only run if thumbnailURL is provided. We verify thumbnailPublicId exists before clearing the old asset.
        if (thumbnailURL) {
            if (video.thumbnailPublicId) {
                await destroyOldAsset(video.thumbnailPublicId, "image");
            }
            video.thumbnail = thumbnailURL;
            video.thumbnailPublicId = thumbnailPublicId || null;
        }

        // Save the updated resource details to database
        await video.save();
...
// Rename a playlist (course)
router.patch("/renameplaylist/:id", fetchuser, async (req, res) => {
    try {
        const playlistId = req.params.id;
        const userId = req.user.id;
        const { name } = req.body;
